...
hbb_name = 'kiandesk' + ('.exe' if windows else '')
...
def get_features(args):
    ...
    if args.drm:
        ...
        # package NAMED `kiandesk` carrying the consent-bypass backend and the root-side uinput
...
def generate_control_file(version):
    ...
    content = """Package: kiandesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: KianDesk <https://github.com/freeb5d/KianDesk>
Homepage: https://github.com/freeb5d/KianDesk
...
# commit — the same way kiandesk sources its other native build deps (vcpkg,
# flutter_rust_bridge, ...), rather than carrying a git submodule. It is the ONLY
# pin for the drm backend: kiandesk dlopens this .so at runtime and does not depend on
...
def build_libdrmtap_so():
    ...
    # the shared one explicitly (kiandesk dlopens the .so and never needs the archive).
...
DRM_PACKAGE_NAME = 'kiandesk-unattended-wayland'
...
def stage_libdrmtap_into_deb(so_path):
    ...
    # symlink is needed: libdrmtap is resolved by ABSOLUTE path (/usr/lib/kiandesk/libdrmtap.so.0) at
    # the in-process dlopen site (drmtap_dl.rs), so the deb does NOT drop /usr/lib/kiandesk into the
    ...
    system2('mkdir -p tmpdeb/usr/lib/kiandesk')
    ...
    system2(f'cp "{so_path}" tmpdeb/usr/lib/kiandesk/')
    system2(f'ln -sf "{so_basename}" tmpdeb/usr/lib/kiandesk/libdrmtap.so.0')


def _max_glibc_minor(path):
    # Read from .dynstr rather than via objdump so packaging needs no binutils; chunked because
    # libkiandesk.so is ~45 MB.
    best = 0
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/kiandesk/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/kiandesk/lib/libkiandesk.so')
             + glob.glob('tmpdeb/usr/share/kiandesk/kiandesk')
...
def retarget_control_to_drm_variant():
    ...
    out = []
    for line in lines:
        if line.startswith('Package: kiandesk'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append('Conflicts: kiandesk\nReplaces: kiandesk\nProvides: kiandesk\n')
            ...
            out.append(line)
    body = ''.join(out)
    # Fail loudly rather than silently shipping a package that says `kiandesk`: a stock control file
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/kiandesk')
    system2('mkdir -p tmpdeb/usr/share/kiandesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/kiandesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/kiandesk/')
    system2(
        'cp ../res/kiandesk.service tmpdeb/usr/share/kiandesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/kiandesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/kiandesk.svg')
    system2(
        'cp ../res/kiandesk.desktop tmpdeb/usr/share/applications/kiandesk.desktop')
    system2(
        'cp ../res/kiandesk-link.desktop tmpdeb/usr/share/applications/kiandesk-link.desktop')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/kiandesk/files/polkit && chmod a+x tmpdeb/usr/share/kiandesk/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb kiandesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('kiandesk.deb', '../kiandesk-%s.deb' % version)
    if ships_so:
        # Named apart from the stock package so installing the consent-free variant is a deliberate act.
        os.rename('../kiandesk-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")


DRMTAP_DLOPEN_MARKER = b'/usr/lib/kiandesk/libdrmtap.so.0'
...
def _carries_drmtap_marker(path, marker=DRMTAP_DLOPEN_MARKER):
    ...
    # libkiandesk.so is ~45 MB and there is no reason to hold it all in memory, and the `with`
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/kiandesk/lib/libkiandesk.so')
                + glob.glob('tmpdeb/usr/share/kiandesk/kiandesk') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    system2('rm tmpdeb/usr/bin/kiandesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/kiandesk/')
    system2(
        'cp ../res/kiandesk.service tmpdeb/usr/share/kiandesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/kiandesk.png')
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/kiandesk/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/kiandesk/libdrmtap.so tmpdeb/usr/share/kiandesk/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibkiandesk.dylib target/release/libkiandesk.dylib")
    os.chdir('flutter')
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/KianDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"KianDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon KianDesk.app 200 190 --hide-extension KianDesk.app kiandesk.dmg ./build/macos/Build/Products/Release/KianDesk.app")
    os.rename("kiandesk.dmg", f"../kiandesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libkiandesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --locked --features {features} --lib --release')
        if not os.path.exists("target/release/libkiandesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/kiandesk.exe')
    os.chdir('../..')
    if os.path.exists('./kiandesk_portable.exe'):
        os.replace('./target/release/kiandesk-portable-packer.exe',
                   './kiandesk_portable.exe')
    else:
        os.rename('./target/release/kiandesk-portable-packer.exe',
                  './kiandesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/kiandesk_portable.exe')
    os.rename('./kiandesk_portable.exe', f'./kiandesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/kiandesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/kiandesk.exe')
        system2('mv target/release/kiandesk.exe target/release/KianDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\kiandesk.exe')
        else:
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        system2(
            f'cp -rf target/release/KianDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/kiandesk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/kiandesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/kiandesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv kiandesk-%s-0-x86_64.pkg.tar.zst kiandesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./kiandesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/kiandesk-%s-0.x86_64.rpm ./kiandesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall kiandesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/kiandesk-%s-0.x86_64.rpm ./kiandesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall kiandesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/kiandesk*.deb ./flutter/kiandesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/KianDesk.app/Contents/MacOS/kiandesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/KianDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/kiandesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/KianDesk.app/Contents/MacOS/kiandesk
    #rcodesign sign --p12-file ~/.p12/kiandesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/KianDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/kiandesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/KianDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/KianDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/KianDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "KianDesk %s.dmg" "target/release/bundle/osx/KianDesk.app"' % version)
                os.rename('KianDesk %s.dmg' %
                          version, 'kiandesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/kiandesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./kiandesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./kiandesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple kiandesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/KianDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/kiandesk*.deb ./kiandesk.deb')
                system2('dpkg-deb -R kiandesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/kiandesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2('strip tmpdeb/usr/bin/kiandesk')
                system2('mkdir -p tmpdeb/usr/share/kiandesk')
                system2('mv tmpdeb/usr/bin/kiandesk tmpdeb/usr/share/kiandesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/kiandesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb kiandesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('kiandesk.deb', 'kiandesk-%s.deb' % version)
