...
// KianDesk defaults to Persian when the user hasn't explicitly picked a
// language (no saved "lang" option), regardless of the system locale.
const DEFAULT_LANG: &str = "fa";
...
mod test {
    ...
    #[test]
    fn test_resolve_lang_defaults_to_persian_when_unset() {
        use super::resolve_lang as f;

        // No saved preference: default to Persian regardless of system locale.
        assert_eq!(f("", "en-US", false), "fa");
        assert_eq!(f("", "zh_CN", false), "fa");
        assert_eq!(f("", "ja-JP", false), "fa");
    }
