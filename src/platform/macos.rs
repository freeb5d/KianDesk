...
// remove just one app from all the permissions: tccutil reset All com.kiandesk.kiandesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.kiandesk.kiandesk", &bundleid);
    }
...
pub fn write_plists() -> ResultType<()> {
    let daemon_plist_path = format!(
        "/Library/LaunchDaemons/com.kiandesk.{}_service.plist",
        crate::get_app_name()
    );
    let agent_plist_path = format!(
        "/Library/LaunchAgents/com.kiandesk.{}_server.plist",
        crate::get_app_name()
    );
    let Some(daemon_plist) = PRIVILEGES_SCRIPTS_DIR.get_file("daemon.plist") else {
...
pub fn update_from_dmg_as_root(dmg_path: &str, expected_version: &str) -> ResultType<()> {
    ...
    let agent_plist = format!("/Library/LaunchAgents/com.kiandesk.{}_server.plist", app_name);
    let daemon_plist = format!("/Library/LaunchDaemons/com.kiandesk.{}_service.plist", app_name);
    ...
    let daemon_label = format!("com.kiandesk.{}_service", app_name);
    let agent_label = format!("com.kiandesk.{}_server", app_name);
