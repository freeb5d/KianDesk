...
pub async fn get_key(_sync: bool) -> String {
    // Server key is fixed at compile time; ignore any user/runtime override.
...
pub fn get_builtin_option(key: &str) -> String {
    // Server is fixed at compile time; always hide the server/proxy/websocket
    // settings so users can't point the client at a different server.
    if key == "hide-server-settings" || key == "hide-proxy-settings" || key == "hide-websocket-settings" {
        return "Y".to_owned();
    }
