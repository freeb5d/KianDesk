...
const APP_PREFIX: &str = "kiandesk";
const APPNAME_RUNTIME_ENV_KEY: &str = "RUSTDESK_APPNAME";
#[cfg(windows)]
const SET_FOREGROUND_WINDOW_ENV_KEY: &str = "SET_FOREGROUND_WINDOW";

// The extraction directory follows whatever executable the payload asks for, so a
// custom client gets its own directory instead of sharing KianDesk's. Falls back to
...
mod win {
    ...
    pub const WIN_TOPMOST_INJECTED_PROCESS_EXE: &'static str = "RuntimeBroker_kiandesk.exe";
    ...
    pub(super) fn copy_runtime_broker(dir: &Path) {
        ...
        let _allow_err = Command::new("taskkill")
            .args(&["/F", "/IM", "RuntimeBroker_kiandesk.exe"])
