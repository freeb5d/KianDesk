...
typedef char** (*FUNC_KIANDESK_CORE_MAIN)(int*);
typedef void (*FUNC_KIANDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_KIANDESK_GET_APP_NAME)(wchar_t*, int);
typedef int (*FUNC_KIANDESK_IS_DISABLE_INSTALLATION)();
...
{
  HINSTANCE hInstance = LoadLibraryA("libkiandesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libkiandesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_KIANDESK_CORE_MAIN kiandesk_core_main =
      (FUNC_KIANDESK_CORE_MAIN)GetProcAddress(hInstance, "kiandesk_core_main_args");
  if (!kiandesk_core_main)
  {
    std::cout << "Failed to get kiandesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_KIANDESK_FREE_ARGS free_c_args =
      (FUNC_KIANDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = kiandesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
  ...
  FUNC_KIANDESK_IS_DISABLE_INSTALLATION kiandesk_is_disable_installation =
      (FUNC_KIANDESK_IS_DISABLE_INSTALLATION)GetProcAddress(hInstance, "kiandesk_is_disable_installation");
  bool is_disable_installation =
      kiandesk_is_disable_installation && kiandesk_is_disable_installation() != 0;
  ...
  std::wstring app_name = L"KianDesk";
  FUNC_KIANDESK_GET_APP_NAME get_kiandesk_app_name = (FUNC_KIANDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_kiandesk_app_name");
  if (get_kiandesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_kiandesk_app_name(app_name_buffer, 512) == 0) {
