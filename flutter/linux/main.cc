...
#define KIANDESK_LIB_PATH "libkiandesk.so"
typedef bool (*KianDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

// The bundle keeps the core library at lib/libkiandesk.so next to the
...
static void* dlopen_bundled_lib() {
  ...
  *last_slash = '\0';
  char lib_path[PATH_MAX + sizeof("/lib/" KIANDESK_LIB_PATH)];
  snprintf(lib_path, sizeof(lib_path), "%s/lib/%s", exe_path, KIANDESK_LIB_PATH);
  if (access(lib_path, F_OK) != 0) return nullptr;
  void* libkiandesk = dlopen(lib_path, RTLD_LAZY);
  if (!libkiandesk) {
    char* error = dlerror();
    if (error != nullptr) {
...
bool flutter_kiandesk_core_main() {
   void* libkiandesk = dlopen_bundled_lib();
   if (!libkiandesk) {
      libkiandesk = dlopen(KIANDESK_LIB_PATH, RTLD_LAZY);
   }
   if (!libkiandesk) {
      fprintf(stderr,"Failed to load \"libkiandesk.so\"\n");
      char* error;
   ...
   auto core_main = (KianDeskCoreMain) dlsym(libkiandesk,"kiandesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"kiandesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
