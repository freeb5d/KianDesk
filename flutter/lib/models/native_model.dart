...
/// The Linux bundle keeps the core library at lib/libkiandesk.so next to the
...
DynamicLibrary _openLinuxCoreLib() {
  final bundled =
      '${File(Platform.resolvedExecutable).parent.path}/lib/libkiandesk.so';
  try {
  ...
  return DynamicLibrary.open('libkiandesk.so');
}
...
class PlatformFFI {
  ...
  KiandeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libkiandesk.so')
        : isLinux
            ? _openLinuxCoreLib()
            : isWindows
                ? DynamicLibrary.open('libkiandesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibkiandesk.dylib") :
  ...
  void _startListenEvent(KiandeskImpl rustdeskImpl) {
    final appType =
